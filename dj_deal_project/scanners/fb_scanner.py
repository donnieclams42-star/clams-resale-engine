...
LOOSE_SIGNAL_WORDS = [
    "cracked", "broken", "no power", "not working", "for parts", "parts only", "untested", "as is",
    "locked", "icloud", "bundle", "lot", "garage sale", "estate sale", "moving sale", "must sell",
    "need gone", "make offer", "pickup today",
]
...
def _text_has_loose_signal(text: str, keyword: str = "") -> bool:
    ...
    return any(word in text or word in keyword for word in LOOSE_SIGNAL_WORDS)
...
def scan_facebook():
    deals = []
    seen_links = set()
    for keyword in get_keywords_for_cycle('facebook')[:4]:
