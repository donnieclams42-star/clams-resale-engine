...
try:
    from dj_deal_project.config import EBAY_MARKET, HTTP_TIMEOUT, MAX_PRICE, MIN_PRICE
    ...
    from config import EBAY_MARKET, HTTP_TIMEOUT, MAX_PRICE, MIN_PRICE
...
def _build_deal(item: dict, keyword: str, baseline: float):
...
def scan_ebay() -> list[dict]:
    ...
    for keyword in get_keywords_for_cycle("ebay"):
        ...
        for item in items:
            ...
            if not deal or deal['link'] in seen_links:
                continue
