...
async def landing(request: Request):
    ...
    # --- TEMU CLEAN FILTER ---
    try:
        if isinstance(temu_flips, list):
            temu_flips = [x for x in temu_flips if float(x.get("profit",0)) > 0 and float(x.get("buy_price",0)) > 0]
            temu_flips = sorted(temu_flips, key=lambda x: x.get("profit",0), reverse=True)[:10]
    except Exception:
        pass
...
def _build_temu_flip_from_query(seed: dict):
    ...
    return {
        ...
        "temu_url": f"https://www.temu.com/search_result.html?search_key={search_q}",
...
def _normalize_temu_flip_item(item) -> dict:
    ...
    return {
        ...
        "score": round(score, 2),
        "temu_url": str(item.get("temu_url") or item.get("temu_search_url") or ""),
        "temu_search_url": str(item.get("temu_search_url") or item.get("temu_url") or ""),
...
async def admin_user_access(
    ...
    try:
        ...
        target_email = (target_email or "").strip().lower()
        membership = (membership or "").strip().upper()

        if membership not in ["FREE", "PRO", "RESELLER"]:
            return RedirectResponse(f"/admin?email={email}&error=Invalid+membership", status_code=303)

        update_user_record(target_email, {"membership": membership})

        return RedirectResponse(f"/admin?email={email}&notice=Saved", status_code=303)
